...
def parse_args():
    ...
    parser.add_argument("--label_ratio", type=float, default=1.0,
                        help="Fraction of GT labels used during pretraining (1.0=fully supervised, "
                             "0.5/0.2=semi-supervised with CatBoost pseudo-labels for the rest). "
                             "Requires gen_pseudo_labels.py to have been run first.")
    parser.add_argument("--split_suffix", type=str, default='',
                        help="Suffix appended to all split CSV filenames, e.g. '_clean' to use "
                             "train_clean.csv / val_clean.csv / test_clean.csv.")
    args = parser.parse_args()

    ratio_tag = '' if args.label_ratio == 1.0 else f'_semi{args.label_ratio}'
    setting = f'seed{args.seed}{ratio_tag}{args.split_suffix}_{args.dataset}'
...
def main(args):
    ...
    train_suffix = '' if args.label_ratio == 1.0 else f'_semi{args.label_ratio}'
    ...
    if args.dataset == 'composite':
        ...
        trainfile = f'../datasets_composite/train{train_suffix}{split_suffix}.csv'
        testfile = f'../datasets_composite/test{split_suffix}.csv'
        valfile = f'../datasets_composite/val{split_suffix}.csv'
        ...
        trainfile = f'../datasets_fiber/table/mech/train{train_suffix}{split_suffix}.csv'
        testfile = f'../datasets_fiber/table/mech/test{split_suffix}.csv'
        valfile = f'../datasets_fiber/table/mech/val{split_suffix}.csv'
